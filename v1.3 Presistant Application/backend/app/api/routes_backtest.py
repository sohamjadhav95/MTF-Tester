...
from app.db import save_credentials, load_credentials, clear_credentials, save_session, load_session
...
async def api_save_credentials(req: SaveCredentialsRequest):
    """Save MT5 credentials to local database."""
    save_credentials(req.server, req.login, req.password)
    return {"success": True}


@router.get("/credentials/load")
async def api_load_credentials():
    """Load saved MT5 credentials."""
    creds = load_credentials()
    if creds is None:
        return {"saved": False}
    return {"saved": True, **creds}


@router.delete("/credentials/clear")
async def api_clear_credentials():
    """Remove saved MT5 credentials."""
    clear_credentials()
    return {"success": True}
...
async def api_load_session():
    ...
    if session is None:
        return {"saved": False}
    return {"saved": True, "config": session}
