...
async function runBacktest() {
  ...
  loadingSub.textContent = 'Initializing...';
  ...
  // Generate a random task ID for progress tracking
  const taskId = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2);

  // Poll progress every 500ms
  const progressInterval = setInterval(async () => {
    try {
      const res = await api(`/api/progress/${taskId}`);
      if (res && res.status !== 'not_found' && res.status !== 'error') {
        let pct = 0;
        if (res.total > 0) {
          pct = Math.round((res.current / res.total) * 100);
        }
        progressBar.style.width = `${pct}%`;
        loadingPct.textContent = `${pct}%`;
        if (res.message) {
          loadingSub.textContent = res.message;
        }
      }
    } catch (e) {
  ...
  try {
    ...
    loadingSub.textContent = 'Rendering results...';
